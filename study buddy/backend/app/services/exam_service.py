...
def generate_exam_questions(
    ...
    for _ in range(max(1, attempts)):
        ...
        for model_name in ["gemini-3.6-flash", "gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash"]:
            try:
                logger.info(f"[AI DEBUG] Generating exam questions with model '{model_name}' on Key #{key_idx + 1}")
                ...
                logger.error(f"[AI ERROR] Exam Generation API error for model '{model_name}' on Key #{key_idx + 1}: {str(e)}")
