...
def generate_tutor_response(
    ...
    # Candidate models pool in order of preference (fastest/most reliable official Gemini models)
    ...
    if not client or not key_manager.keys:
        logger.warning("[AI DEBUG] Gemini API key loaded: NO")
        return "⚠️ **Gemini API Key Missing**: Please add `GEMINI_API_KEY=your_key_here` to `backend/.env` file and try again!"

    attempts = len(key_manager.keys)
    ...
    for attempt_num in range(attempts):
        ...
        for model_name in candidate_models:
            try:
                ...
                logger.error(f"[AI ERROR] Gemini request failed for model '{model_name}' on Key #{key_idx + 1}: {last_error_msg}")
        
        # If all models failed for this key, mark key for short cooldown
        if key_idx is not None:
            key_manager.mark_key_failed(key_idx, duration=15)

    logger.error(f"[AI ERROR] All Gemini API attempts failed: {last_error_msg}")
    return f"⚠️ **AI Service Response Error**: {last_error_msg if last_error_msg else 'Failed to generate response'}. Please verify your `GEMINI_API_KEY` in `backend/.env`!"
...
def generate_conversation_title(first_message: str, language: str = "English") -> str:
    ...
    try:
        ...
        for model_name in ["gemini-3.6-flash", "gemini-2.5-flash", "gemini-2.0-flash", "gemini-1.5-flash"]:
            try:
                response = client.models.generate_content(
                    model=model_name,
