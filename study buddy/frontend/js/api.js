    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (error.message === 'Failed to fetch' || error.name === 'TypeError') {
                console.error(`API Connection Error [${endpoint}]: Could not connect to backend at http://127.0.0.1:8000`);
                throw new Error('Cannot connect to StudyBuddy backend server. Please run run.bat to start the backend on http://127.0.0.1:8000.');
            }
