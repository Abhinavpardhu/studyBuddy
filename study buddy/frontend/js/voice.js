...
const FEMALE_KEYWORDS = [
    'female', 'woman', 'zira', 'heera', 'geeta', 'swara', 'shruti', 
    'sunhi', 'sun-hi', 'neerja', 'anita', 'priya', 'kavya', 'samantha', 
    'victoria', 'karen', 'jenny', 'aria', 'google', 'cortana', 'hazel', 'susan'
];

const MALE_KEYWORDS = [
    'male', 'man', 'david', 'mark', 'george', 'ravi', 'hemant', 
    'madhur', 'prabhat', 'mohan', 'valluvar', 'james', 'richard'
];
...
function getBestVoice(langCode) {
    ...
    // Filter voices for target language
    const langVoices = voices.filter(v => {
        const vLang = v.lang ? v.lang.replace('_', '-').toLowerCase() : '';
    ...
    for (const v of pool) {
        ...
        if (FEMALE_KEYWORDS.some(k => nameLower.includes(k)) && !MALE_KEYWORDS.some(k => nameLower.includes(k))) {
            console.log('[TTS] Selected female voice:', v.name, 'for lang:', langCode);
            return v;
        }
    }

    // Fallback: exclude explicit male voices
    const nonMaleVoices = pool.filter(v => {
        const nameLower = v.name.toLowerCase();
        return !MALE_KEYWORDS.some(k => nameLower.includes(k));
    });

    if (nonMaleVoices.length > 0) {
        console.log('[TTS] Selected fallback voice:', nonMaleVoices[0].name);
...
function toggleSpeech(rawText, language = 'English', btnElement = null) {
    console.log('[TTS] toggleSpeech clicked. isSpeaking:', isSpeaking);

    // If currently speaking on THIS button, stop it
    if (isSpeaking && currentSpeakingBtn === btnElement) {
        console.log('[TTS] User requested stop via button click');
        stopSpeech(btnElement);
        return;
    }

    // Stop any speech playing on another button
    stopSpeech();

    // Start speaking for this button
    speakText(rawText, language, btnElement);
}
...
function speakText(rawText, language = 'English', btnElement = null) {
    ...
    // Cancel existing playback and timers
    if ('speechSynthesis' in window) {
    ...
    currentUtterance.pitch = 1.15; // Set pitch for a natural female voice tone
    ...
    // Chromium keep-alive ping to prevent speech synthesis freeze on long text
...
function stopSpeech(btnElement = null) {
    console.log('[TTS] stopSpeech executed');
    isSpeaking = false;
    if (ttsKeepAliveInterval) {
        clearInterval(ttsKeepAliveInterval);
        ttsKeepAliveInterval = null;
    ...
    if ('speechSynthesis' in window) {
        try {
            ...
            console.error('[TTS] Error during cancel:', e);
        }
    }
